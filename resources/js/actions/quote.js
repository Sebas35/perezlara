...
function add_event(){
    document.querySelectorAll ('.select + .modal .checkbox')
            .forEach (e => e.addEventListener ('input', create_quote_table));
}
...
function create_quote_table(e){
    if (e.target.checked){
    ...
    } else if (e.target.closest ('#coberturas-select')){
        ...
        const remove = Array.from (thead_form.rows[0].children)
...
async function edit(){
    ...
    const insurers = data['id_aseguradora'].split (','), names_insurers = data['nombre_aseguradora'].split (','),
        ...
        price = data['Precio'].split (','), deductible = data['Deducible'].split (','),
        premium_value = data['Valor prima'].split (',');
